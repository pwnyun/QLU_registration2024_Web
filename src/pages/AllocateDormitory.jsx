...
export default function AllocateDormitory () {
  ...
  useEffect(() => {
    const fn = async () => {
      ...
      const roommatesRes = await request({
        ...
        setContractError(roommatesRes.msg || '')
  ...
  return (<>
    <div className="container mx-auto max-w-[750px]">
      ...
      <div className={`p-4 ${contractError ? '' : 'pb-12 mb-12'}`}>
        <table className="mt-10 w-full">
          ...
          <tr className="w-full">
            ...
            <td
              className="border-y border-gray-300 w-full">{dormitoryInformation?.area ||
          ...
          <tr className="w-full">
            ...
            <td
              className="border-b border-gray-300 w-full">{dormitoryInformation?.build
              ? dormitoryInformation.build + '# 学生公寓'
              : <span className="text-gray-600">（暂无信息）</span>}</td>
          </tr>
          <tr className="w-full">
            <th
            ...
            <td
              className="border-b border-gray-300 w-full">{dormitoryInformation?.floor
              ? dormitoryInformation.floor + '层'
              : <span className="text-gray-600">（暂无信息）</span>}</td>
          </tr>
          <tr className="w-full">
            <th
            ...
            <td
              className="border-b border-gray-300 w-full">{dormitoryInformation?.dormitory ||
              <span className="text-gray-600">（暂无信息）</span>}</td>
          </tr>
          <tr className="w-full">
            <th
            ...
            <td
              className="border-y border-gray-300 w-full">{dormitoryInformation?.bed_number ||
              <span className="text-gray-600">（暂无信息）</span>}</td>
          </tr>
          </tbody>
        </table>
      ...
      {!contractError || (Array.isArray(roommatesInformation) && roommatesInformation.length !== 0) ? (
          ...
          <table className="mt-4 w-full">
            ...
            {roommatesInformation.map(roommate => (
              ...
              <tr className="w-full" key={roommate.id_card}>
                <td
                  className="border-b border-gray-300 text-center py-2 px-3">{roommate.name}</td>
                <td
                  className="border-b border-gray-300 text-center py-2 px-3"></td>
                <td
                  className="border-b border-gray-300 text-center py-2 px-3">{roommate.phone ||
                  <span className="text-gray-400">未共享</span>}</td>
                <td
                  className="border-b border-gray-300 text-center py-2 px-3">{roommate.qq ||
                  <span className="text-gray-400">未共享</span>}</td>
              </tr>
            ))}
            </tbody>
