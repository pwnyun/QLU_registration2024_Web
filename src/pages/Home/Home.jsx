...
import { encrypt, formDataToJson, validateIdCard } from '../../libs/utils.js'
import { VscClose, VscScreenFull } from 'react-icons/vsc'
import CountDownModal from '../../components/count-down-modal.jsx'
...
export default function Home() {
  ...
  const [isIPhone] = useState(
    /Safari/.test(navigator.userAgent) && /iPhone/.test(navigator.userAgent))
  ...
  const checkAndLogin = () => {
    ...
    if (!name) {
      ...
      setShowModal(true)
      setModalContent(<>居民身份证号无效。<br />如您使用其他类型证件，请联系网络信息中心：0531-89631358。</>)
      setModalButtonText('关闭')
      setModalOptionalButton(null)
      return
    }
    ...
    request({
      url: '/api/login',
      method: 'POST',
      data: { id_card: idCard, name },
    }).then(res => {
      if (res.code === 200) {
        Promise.all([
          localforage.setItem('login_info', encrypt({ name, idCard, token: res.token })),
  ...
  const handleShowWiki = () => {
    window.location.href = 'https://wlyw.qlu.edu.cn/wiki/2025yx/'
    // setShowModal(true)
  ...
  const antiCheatTips = <>
    <div className={`${!isIPhone
      ? 'fixed bottom-0 left-0 h-48'
      : 'order-last'} ${showModal
        ? 'hidden'
        : 'flex'} w-full lg:max-w-72 items-end justify-center lg:static lg:size-auto lg:bg-none`}>
      <div
        className="pointer-events-none flex flex-col place-items-center p-8 text-white border-orange-100 lg:pointer-events-auto lg:px-4 lg:py-2 lg:border-2 lg:rounded"
      >
        <div className="font-serif font-bold text-base py-1">反诈提示</div>
        <div className="w-full indent-8">
          请广大新生请勿加入或关注其他任何非官方建立的“QQ
          群”等，不要在群内提交任何个人信息，谨防电信诈骗。
        </div>
        <div className="w-full indent-8">
          预报到过程中遇到问题，可加入线上预报到服务 QQ 群：725386079
        </div>
      </div>
    </div>
  </>
  ...
  return (
    <div id="homepage_main"
      className="bg-[url('/assets/bg-red-1.png')] bg-center bg-cover">
      <main
        className={`font-sans flex min-h-screen flex-col items-center ${!isIPhone
          ? 'justify-between'
          : 'justify-start gap-y-4'}  px-8 md:px-24 py-24 backdrop-brightness-110 backdrop-contrast-110 backdrop-saturate-110`}>
        <div
          className="z-20 w-full max-w-5xl items-start justify-between text-sm lg:flex">
          <div
            className="fixed text-lg text-center lg:text-base left-0 top-0 flex w-full justify-center items-center text-white gap-x-2  border-b border-gray-300 pb-6 pt-8 backdrop-blur-2xl lg:static lg:w-auto lg:rounded-xl lg:border-0 lg:p-4">
            <img
              className="relative block drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] w-16 z-10"
              src="/assets/qlu-logo-space-white.svg"
              alt="齐鲁工大（鲁科院）Logo"
            />
            <div className="flex flex-col items-start font-sans">
              <div className="text-sm sm:text-lg lg:text-bsae">齐鲁工业大学
                <wbr />
                （山东省科学院）
              </div>
              <div className="sm:text-lg lg:text-bsae"><span
                className="font-mono pr-1">2025</span>新生线上报到
              </div>
            </div>
          </div>
        ...
        <div
          className={`${!isIPhone &&
            'mb-32'} grid text-center lg:mb-0 lg:w-full lg:max-w-5xl lg:grid-cols-4 lg:text-left text-orange-50`}>

          <div
            className="group rounded-lg border border-transparent px-2 md:px-5 py-4 transition-colors hover:border-gray-300 hover:bg-gray-100/30 hover:backdrop-blur cursor-pointer select-none"
            onClick={handleLogin}
          >
      ...
      <Modal isOpen={showModal} setIsOpen={(status) => setShowModal(status)}
        title={modalTitle}
      ...
      <CountDownModal isOpen={showCountDownModal}
        setIsOpen={(status) => setShowCountDownModal(status)}
        countdownSeconds={3}>
        倒计时提示，3 秒后可关闭
      </CountDownModal>
    </div>)
    ...
    <form ref={ref} {...props} >
      <div className="border-b border-gray-900/10 pt-4 pb-8 z-20">
        <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-1">
          ...
          {/*    <img src={captcha}*/}
          {/*         className="bg-amber-500 w-auto h-full flex-grow text-nowrap"*/}
          {/*         alt="验证码"*/}
          {/*         onClick={() => setCaptcha}/>*/}
          {/*    <input*/}
          {/*      type="text"*/}
