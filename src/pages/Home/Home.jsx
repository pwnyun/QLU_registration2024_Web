...
import CountDownModal from "../../count-down-modal.jsx";
...
export default function Home() {
  ...
  const antiCheatTips = <>
    <div
      className={`${!isIPhone ? 'fixed bottom-0 left-0 h-48' : 'order-last'} ${showModal ? 'hidden' : 'flex'} w-full lg:max-w-72 items-end justify-center lg:static lg:size-auto lg:bg-none`}>
      <div
  ...
  return (
    <div id="homepage_main" className="bg-[url('/assets/bg-red-1.png')] bg-center bg-cover">
      <main
        className={`font-sans flex min-h-screen flex-col items-center ${!isIPhone ? 'justify-between' : 'justify-start gap-y-4'}  px-8 md:px-24 py-24 backdrop-brightness-110 backdrop-contrast-110 backdrop-saturate-110`}>
        ...
        <div
          ...
          <div className="text-orange-100/90 font-serif text-4xl sm:text-5xl lg:text-6xl lg:text-orange-50 text-shadow-lg/30 font-bold">
            欢迎新同学
          </div>
          {/*<img*/}
        ...
        <div
          className={`${!isIPhone && 'mb-32'} grid text-center lg:mb-0 lg:w-full lg:max-w-5xl lg:grid-cols-4 lg:text-left text-orange-50`}>
      ...
      <CountDownModal isOpen={showCountDownModal} setIsOpen={(status) => setShowCountDownModal(status)} countdownSeconds={3}>倒计时提示，3 秒后可关闭</CountDownModal>
    </div>
  )
}
